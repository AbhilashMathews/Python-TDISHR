...
_lex=_mimport('lex',1)
_yacc=_mimport('yacc',1)
_builtin_set=_mimport('builtin_set',1)
try:
    _tdilextab=_mimport('tdilextab',1)
except:
    _tdilextab='tdilextab'
try:
    _tdiparsetab=_mimport('tdiparsetab',1)
except:
    _tdiparsetab='tdiparsetab'
...
class COMPILE(_builtin_set.Builtin):
    """      Compile Operation. 
    ...
    def _evaluate(self):
        text=str(self.args[0])
        replacementArgs=_replacementArgs(self.args[1:])
...
### Reserved keywords
        ...
        reserved = {'if':'IF','else':'ELSE','public':'IDENTTYPE',
                    'private':'IDENTTYPE','fun':'FUN','in':'ARG_TYPE',
                    'out':'ARG_TYPE','inout':'ARG_TYPE','optional':'ARG_TYPE',
                    'as_is':'AS_IS','switch':'SWITCH','case':'CASE',
...
### integer token including hex,binary,octal and decimal
        ...
        def fix_backquotes(in_str):
            ...
            def replace_backquote_string(match):
                ...
                return ans
            return re.sub(r'\\[0-7]+|\\[\\mntr]',replace_backquote_string,in_str)

### string token with double quotes converted to String() instance
        @_lex.TOKEN(r'"(?:[^"\\]|\\.)*"')
        def t_T(t):
            t.value=_data.String(fix_backquotes(t.value).replace('\\"','"').replace("\\'","'").replace('\\\\','\\')[1:-1])
            return t
...
### unsigned byte token converted to Uint8() instance
        ...
        def t_BU(t):        
            t.value=_data.Uint8(int(t.lexer.lexmatch.group('number1') or t.lexer.lexmatch.group('number2'),0))
            return t
...
### unsigned word converted to Uint16() instance
        ...
        def t_WU(t):
            t.value=_data.Uint16(int(t.lexer.lexmatch.group('number1') or t.lexer.lexmatch.group('number2'),0))
            return t

### signed word converted to Int16() instance
        @_lex.TOKEN(r'(?i)word\((?P<number1>('+integer+r'))\)|(?P<number2>('+integer+r'))w')
        def t_W(t):
            t.value=_data.Int16(int(t.lexer.lexmatch.group('number1') or t.lexer.lexmatch.group('number2'),0))
            return t
...
### unsigned quadword converted to Uint64() instance
        ...
        def t_QU(t):
            t.value=_data.Uint64(int(t.lexer.lexmatch.group('number1') or t.lexer.lexmatch.group('number2'),0))
            return t
...
### unsigned int converted to Uint32() instance
        ...
        def t_LU(t):
            t.value=_data.Uint32(int(t.lexer.lexmatch.group('number1') or t.lexer.lexmatch.group('number2'),0))
            return t

### signed quadword converted to Int64() instance
        @_lex.TOKEN(r'(?i)quadword\((?P<number1>('+integer+r'))\)|(?P<number2>('+integer+r'))q')
        def t_Q(t):
            t.value=_data.Int64(int(t.lexer.lexmatch.group('number1') or t.lexer.lexmatch.group('number2'),0))
            return t
...
### Float instance converted to either Float32() or Float64() instance
        ...
        def t_FloatNum(t):
            ...
            if exp is None or exp == 'e' or exp == 'f':
                t.value=_data.Float32(val)
            else:
                t.value=_data.Float64(val)
...
### signed byte converted to Int8() instance
        ...
        def t_B(t):
            t.value=_data.Int8(int(t.lexer.lexmatch.group('number1') or t.lexer.lexmatch.group('number2'),0))
            return t
...
### signed int converted to Int32() instances. NOTE must be end of the scalar tokens to work for some reason.
        ...
        def t_L(t):
            t.value=_data.Int32(int(t.lexer.lexmatch.group('number1') or t.lexer.lexmatch.group('number2'),0))
            return t
...
### Ident or builtin constant converted to either Ident() instance or a Builtin() instance for constants such as $PI
        ...
        def t_IDENT(t):
          if t.value.lower()=="$roprand":
            t.value=_data.Float32(_NP.nan)
          else:
            try:
              t.value=_builtin_set.Builtin(t.value,())
            except Exception:
              t.value=_data.Ident(t.value)
          return t
...
# Build the lexer
        ...
        _lex.lex(debug=0,optimize=optimized,lextab=_tdilextab)
        ...
        precedence = (
            ...
            ('left','COLON'),
            ('left','QUESTION'),
            ('left','OR','AND','OR_S','AND_S','AND_NOT_S'),
...
### Subscripting (i.e. _a[32])
        def p_subscript(t):
            ...
            if len(t) == 2:
                ...
                args=[t[1],]
                if isinstance(t[2],_builtin_set.Builtin):
                    for arg in t[2].args:
                        args.append(arg)
                else:
                    for arg in t[2]:
                        args.append(arg)
                t[0]=_builtin_set.Builtin('subscript',tuple(args))
...
### Ken variable (i.e. _gub or public _gub)
        def p_ident(t):
            ...
            if len(t) == 2:
                ...
                t[0]=_builtin_set.Builtin(t[1],(str(t[2]),))
...
### Unary arithmetic operations such as ~a, -a
        def p_unaryop(t):
            ...
            if t[1]=='-' and isinstance(t[2],_data.Scalar):
                t[0]=_data.makeData(-t[2].data())
            elif t[1]=='+' and isinstance(t[2],_data.Scalar):
                t[0]=t[2]
            else:
                t[0]=_builtin_set.Builtin(ops[t[1].lower()],(t[2],))
...
### Binary arithmetic operations such as a+b a>=b a^b a&&b
        def p_binop(t):
            ...
            ops = {'+':'add','-':'subtract','*':'multiply','/':'divide','<':'lt',
                   ...
                   '|':'ior','||':'or','and':'and','and_not':'and_not','or':'or','nor':'nor','mod':'MOD',
                   'gt':'gt','ge':'ge','lt':'lt','le':'le','eq':'eq','ne':'ne'}
            if len(t)==4:
                t[0]=_builtin_set.Builtin(ops[t[2].lower()],(t[1],t[3]))
            else:
                t[0]=_builtin_set.Builtin(ops[t[1].lower()],(t[3],t[5]))
...
### items being concatenated is not a string then don't concat anything at run time.
        class Concat(list):
            def get(self):
                ...
                for arg in self:
                    if not isinstance(arg,(str,_data.String)):
                ...
                if compile_time_concat:
                    ...
                    if len(self) % 2 == 0:
                        ...
                        for arg in self[1:]:
                            c[-1]=_data.String(str(c[-1])+str(arg))
                    if len(c)>1:
                        return _builtin_set.Builtin('concat',tuple(c))
                    else:
                        return c[0]
                else:
                    return _builtin_set.Builtin('concat',tuple(self))
...
### Ident increment/decrement (i.e. _i++, _i--, ++_i, --_i)
        def p_inc_dec(t):
            ...
            if isinstance(t[1],str):
                t[0]=_builtin_set.Builtin('pre'+op[t[1]],(t[2],))
            else:
                t[0]=_builtin_set.Builtin('post'+op[t[2]],(t[1],))

### Ken variable assignment (i.e. _i=1)
        def p_assignment(t):
            'operation : operand EQUAL operand %prec EQUAL'
            t[0]=_builtin_set.Builtin('EQUALS',(t[1],t[3]))
...
### Argument list for function calls (i.e. ([a[,b[,c]]])  )
        def p_arglist(t):
            ...
            if len(t)==4:
                ...
                if len(t)==6:
                    t[2]=_builtin_set.Builtin(t[2],(t[4],))
        ...
        def p_function(t):
            ...
            def doBuild(name,args):
                ...
                def build_slope(args):
                    ...
                    return _data.Range(tuple(new_args))

                def buildPath(args):
                    if isinstance(args[0],(str,_data.String)):
                        name=str(args[0])
                        if len(name) > 1 and name[0:2]=='\\\\':
                            name=name[1:]
                        ans = _treenode.TreePath(name)
                    else:
                        ans = _builtin_set.Builtin('build_path',args)
                    return ans
            
                def buildCall(args):
                    ans=_data.Call(args[1:])
                    ans.retType=args[0]
                    return ans
...
### retain original node specifiers when building a using function
                def buildUsing(args_in):
                    def restoreTreePaths(arg):
                        if isinstance(arg,_data.Compound):
                            args=list()
                            for a in arg.args:
                                args.append(restoreTreePaths(a))
                            arg.args=tuple(args)
                            ans = arg
                        elif isinstance(arg,(_treenode.TreePath,_treenode.TreeNode)) and hasattr(arg,'original_value'):
                            ans = _treenode.TreePath(arg.original_value)
                        else:
                            ans = arg
                        return ans
                    ...
                    ans = _builtin_set.Builtin('using',tuple(args))
                    return ans

                def buildComplex(args_in):
                    ans=_builtin_set.Builtin('cmplx',args_in) 
                    if isinstance(args_in[0],_data.Scalar) and isinstance(args_in[1],_data.Scalar):
                        ans=ans.evaluate()
                    return ans

                def buildDispatch(args_in):
                    return Dispatch(args_in[1:],dispatch_type=int(args_in[0]))

                def buildFunction(args_in):
                    try:
                        return _builtin_set.Builtin(*args)
                    except Exception:
                        print(_sys.exc_info()[1])
                        raise

                known_builds={'BUILD_ACTION':_data.Action,
                              #BUILD_CONDITION':Condition,
                              'BUILD_CONGLOM':_data.Conglom,
                              'BUILD_DEPENDENCY':_data.Dependency,
                              'BUILD_DIM':_data.Dimension,
                              'BUILD_DISPATCH':buildDispatch,
                              #'BUILD_EVENT':Event,
                              'BUILD_FUNCTION':_builtin_set.Builtin,
                              'BUILD_METHOD':_data.Method,
                              'BUILD_PARAM':build_param,
                              'BUILD_PROCEDURE':_data.Procedure,
                              'BUILD_PROGRAM':_data.Program,
                              'BUILD_RANGE':_data.Range,
                              'BUILD_ROUTINE':_data.Routine,
                              'BUILD_SIGNAL':_data.Signal,
                              'BUILD_SLOPE':build_slope,
                              'BUILD_WINDOW':_data.Window,
                              'BUILD_WITH_UNITS':build_with_units,
                              'BUILD_CALL':buildCall,
                              'BUILD_WITH_ERROR':build_with_error,
                              'BUILD_OPAQUE':_data.Opaque,
            ...
            def doMake(name,args):
                for arg in args:
                    if not isinstance(arg,(_data.Array,_data.Scalar,_data.EmptyData)) and arg is not None:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        if t[1].lower() in numbers and (isinstance(t[2][0],_data.Scalar) or isinstance(t[2][0],_data.Array)):
                            t[0]=_data.Data.evaluate(_builtin_set.Builtin(t[1],tuple(t[2])))
                        else:
                            t[0]=_builtin_set.Builtin(t[1],tuple(t[2]))
                    except _data.TdiException:
                        raise
                    except Exception:
                        t[0]=_builtin_set.Builtin('ext_function',tuple([None,t[1]]+t[2]))
...
### call library (i.e. library->gub(a,b,c))
        ...
        def p_call(t):
            ...
            if len(t)==5:
                t[0]=_data.Call(tuple([t[1],t[3]]+t[4]))
            else:
                t[0]=_data.Call(tuple([t[1],t[3]]+t[5]),opcode=t[4])
...
### Loop and fun statements found inside braces and sometimes in parens
        ...
        class CommaList(list):
            def get(self):
                return _builtin_set.Builtin('comma',tuple(self))
        ...
        def p_statements(t):
            ...
            if len(t)==2:
                t[0]=_builtin_set.Builtin('statement',(t[1],))
            else:
                if t[2] is None:
                    t[0]=t[1]
                    ...
                    t[0]=_builtin_set.Builtin('statement',(t[1],t[2]))
...
### For statement (i.e. for (_x=1;_x<10;_x++){statements...} or for (...) statement
        ...
        def p_for(t):
            ...
            t[0]=_builtin_set.Builtin('for',(t[3],t[5],t[7],t[9]))
...
### If statement (i.e. if (_x<10) {_x=42;} else {_x=43;})
        ...
        def p_if(t):
            ...
            if len(t) > 3:
                args.append(t[4])
            t[0]=_builtin_set.Builtin('if',tuple(args))
...
### While statement (i.e. while(expression){statements;} )
        def p_while(t):
            ...
            t[0]=_builtin_set.Builtin('while',(t[3],t[5]))
...
### FUN definition (i.e. public fun gub(args){statements} )
        def p_fun_arg(t):
            ...
            if len(t) == 2:
                ...
                t[0]=_builtin_set.Builtin(t[1],(_builtin_set.Builtin(t[2],(str(t[3]),)),))
            elif len(t) == 5:
                t[0]=_builtin_set.Builtin(t[1],(t[3],))
            else:
                t[0]=_builtin_set.Builtin(t[1],(_builtin_set.Builtin(t[2],(t[4],)),))
        ...
        def p_fun(t):
            ...
            if len(t) == 6:
                if t[1].lower() == 'fun':
                    ...
                    itype=t[1]
                args.append(_builtin_set.Builtin(itype,(t[3],)))
                args.append(t[5])
                for arg in t[4]:
                    args.append(arg)
            else:
                ...
                for arg in t[3]:
                    args.append(arg)
            t[0]=_builtin_set.Builtin('fun',tuple(args))
...
### Vector/Array declarations (i.e. [_a,_b,_c] or [1,2,3,])
        def p_vector(t):
            ...
            if isinstance(t[1],str):
                if len(t)==2:
                    t[0]=_builtin_set.Builtin('vector',tuple())
                    t[0].isarray=True
                else:
                    t[0]=_builtin_set.Builtin('vector',(t[2],))
                    t[0].isarray = isinstance(t[2],_data.Scalar) or isinstance(t[2],_data.Array)
            elif t[2] == ',':
                args=list(t[1].args)
                if len(args) > 250:
                    args=[_builtin_set.Builtin('vector',tuple(args)),t[3]]
                else:
                    args.append(t[3])
                t[1].args=tuple(args)
                t[0]=t[1]
                t[0].isarray = t[1].isarray and (isinstance(t[3],_data.Scalar) or isinstance(t[3],_data.Array))
            else:
                if t[1].isarray:
                    t[0]=_data.Data.evaluate(t[1])
                else:
                    t[0]=_builtin_set.Builtin('vector',t[1].args)
...
### Switch statement (i.e. switch(_a) {case(42) {statements} case(43) {statements}} )
        def p_case(t):
            ...
            if len(t)==4:
                t[0]=_builtin_set.Builtin('default',(t[3],))
            elif len(t)==5:
                t[0]=_builtin_set.Builtin('case',(None,None))
                t[0].args=(t[3],)
                t[0].doAppendCase=True
            elif len(t)==6:
                t[0]=_builtin_set.Builtin('case',(t[3],t[5]))
            else:
                t[0]=t[1]
        ...
        def p_cases(t):
            ...
            def findCaseWithNoStatements(case,parent=None,argidx=0):
                ans=None
                if isinstance(case,_builtin_set.Builtin):
            ...
            def appendCase(cases,case):
                ...
                if len(appendTo['case'].args) < 250:
                    ...
                    statement = _builtin_set.Builtin('statement',(appendTo['case'],case))
        ...
        def p_switch(t):
            ...
            t[0]=_builtin_set.Builtin('switch',(t[3],t[6]))
...
### "Equals First" statement (i.e. _gub+=42)
        def p_operand_equal_first(t):
            ...
            t[0]=_builtin_set.Builtin('equals_first',(_builtin_set.Builtin(ef_dict[t[2]],(t[1],t[3])),))
...
### Handle syntax errors
        ...
        _yacc.yacc(write_tables=optimized,debug=0,optimize=optimized,tabmodule=_tdiparsetab)
        ans = _yacc.parse(text)
        return ans
