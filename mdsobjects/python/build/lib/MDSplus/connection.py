...
__MdsIpShr=_data._loadLib('MdsIpShr')
...
class Connection(object):
    ...
    def __init__(self,hostspec):
      self.socket=_MdsIpShr.ConnectToMds(hostspec.encode())
      if self.socket == -1:
        raise Exception("Error connecting to %s" % (hostspec,))
      self.hostspec=hostspec

    def __del__(self):
        _MdsIpShr.DisconnectFromMds(self.socket)
    ...
    def __sendArg__(self,value,idx,num):
        ...
        val=makeData(value)
        if not isinstance(val,_data.Scalar) and not isinstance(val,_data.Array):
