...
def makeScalar(value):
  ...
  if isinstance(value,_C._SimpleCData) and type(value) in Scalar.ctypesToClass:
    klass=Scalar.ctypesToClass[type(value)]
...
def _scalar_decorator(klass):
  if klass.dtype_numpy not in klass.numpyToClass:
    klass.numpyToClass[klass.dtype_numpy]=klass
...
def makeArray(value):
    ...
    if isinstance(value,_NP.ndarray):
        ...
        return Array.numpyToClass[value.dtype.type](value)
